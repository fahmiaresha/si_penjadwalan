...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 11, 2021 at 07:49 AM
...
INSERT INTO `sewa_bus` (`ID_SEWA_BUS`, `ID_PENGGUNA`, `ID_CUSTOMER`, `EMAIL_CUSTOMER`, `TGL_SEWA_BUS`, `TGL_AKHIR_SEWA`, `JAM_SEWA`, `JAM_AKHIR_SEWA`, `DP_BUS`, `SISA_SEWA_BUS`, `STATUS_SEWA`, `total_payment`, `created_at`, `TGL`, `STATUS`) VALUES
('210611001', 'USR001', 'CUS017', 'v@g', '2021-06-13', '2021-06-14', '15:12:00', NULL, 250000, 750000, 'DIBATALKAN', 1000000, '2021-06-11 08:12:35', '2021-06-11', 0);
...
(188, '210611001', 'HRG138', 1, 0, NULL, 1000000, NULL, NULL);
...
--
-- Stand-in structure for view `view_bus_ready_2`
-- (See below for the actual view)
--
CREATE TABLE `view_bus_ready_2` (
...
--
-- Stand-in structure for view `view_bus_ready_3`
-- (See below for the actual view)
--
CREATE TABLE `view_bus_ready_3` (
`ID_CATEGORY` varchar(10)
,`NAMA_CATEGORY` varchar(20)
,`jmlbis` decimal(43,0)
);
...
--
-- Stand-in structure for view `vw_bus_ready_4`
-- (See below for the actual view)
--
CREATE TABLE `vw_bus_ready_4` (
`ID_CATEGORY` varchar(10)
,`NAMA_CATEGORY` varchar(20)
,`jmlbis` decimal(43,0)
);
...
--
-- Structure for view `view_bus_ready_2`
--
DROP TABLE IF EXISTS `view_bus_ready_2`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_bus_ready_2`  AS SELECT `tmp`.`ID_CATEGORY` AS `ID_CATEGORY`, `tmp`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`, sum(`tmp`.`jmlbis`) AS `jmlbis` FROM (select `ct`.`ID_CATEGORY` AS `ID_CATEGORY`,`ct`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`,count(`ar`.`ID_ARMADA`) AS `jmlbis` from (`armada` `ar` join `category_armada` `ct` on(`ct`.`ID_CATEGORY` = `ar`.`ID_CATEGORY`)) where `ar`.`STATUS_ARMADA` = 1 group by `ct`.`ID_CATEGORY`,`ct`.`NAMA_CATEGORY` union select `ca`.`ID_CATEGORY` AS `ID_CATEGORY`,`ca`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`,-count(`ar`.`ID_ARMADA`) AS `jmlbis` from (((`schedule_armada` `sa` join `armada` `ar` on(`ar`.`ID_ARMADA` = `sa`.`ID_ARMADA`)) join `category_armada` `ca` on(`ca`.`ID_CATEGORY` = `ar`.`ID_CATEGORY`)) join `sewa_bus` `sb` on(`sb`.`ID_SEWA_BUS` = `sa`.`ID_SEWA_BUS`)) where `sa`.`STATUS_ARMADA` = 0 and `sb`.`STATUS_SEWA` <> 'Booking' group by `ca`.`ID_CATEGORY`,`ca`.`NAMA_CATEGORY`) AS `tmp` GROUP BY `tmp`.`ID_CATEGORY`, `tmp`.`NAMA_CATEGORY` ;

-- --------------------------------------------------------

--
-- Structure for view `view_bus_ready_3`
--
DROP TABLE IF EXISTS `view_bus_ready_3`;
...
--
-- Structure for view `vw_bus_ready_4`
--
DROP TABLE IF EXISTS `vw_bus_ready_4`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_bus_ready_4`  AS SELECT `tmp`.`ID_CATEGORY` AS `ID_CATEGORY`, `tmp`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`, sum(`tmp`.`jmlbis`) AS `jmlbis` FROM (select `ct`.`ID_CATEGORY` AS `ID_CATEGORY`,`ct`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`,count(`ar`.`ID_ARMADA`) AS `jmlbis` from (`armada` `ar` join `category_armada` `ct` on(`ct`.`ID_CATEGORY` = `ar`.`ID_CATEGORY`)) where `ar`.`STATUS_ARMADA` = 1 group by `ct`.`ID_CATEGORY`,`ct`.`NAMA_CATEGORY` union select `ca`.`ID_CATEGORY` AS `ID_CATEGORY`,`ca`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`,-count(`ar`.`ID_ARMADA`) AS `jmlbis` from ((`schedule_armada` `sa` join `armada` `ar` on(`ar`.`ID_ARMADA` = `sa`.`ID_ARMADA`)) join `category_armada` `ca` on(`ca`.`ID_CATEGORY` = `ar`.`ID_CATEGORY`)) where `sa`.`STATUS_ARMADA` = 0 group by `ca`.`ID_CATEGORY`,`ca`.`NAMA_CATEGORY` union select `ca`.`ID_CATEGORY` AS `ID_CATEGORY`,`ca`.`NAMA_CATEGORY` AS `NAMA_CATEGORY`,-count(`sa`.`ID_SEWA_BUS`) AS `jmlbis` from (((`sewa_bus` `sa` join `sewa_bus_category` `sbc` on(`sbc`.`ID_SEWA_BUS` = `sa`.`ID_SEWA_BUS`)) join `pricelist_sewa_armada` `psa` on(`psa`.`ID_PRICELIST` = `sbc`.`ID_PRICELIST`)) join `category_armada` `ca` on(`ca`.`ID_CATEGORY` = `psa`.`ID_CATEGORY`)) where `sa`.`STATUS_SEWA` = 'Booking' group by `ca`.`ID_CATEGORY`,`ca`.`NAMA_CATEGORY`) AS `tmp` GROUP BY `tmp`.`ID_CATEGORY`, `tmp`.`NAMA_CATEGORY` ;
